...
# This is an optimized version of the forward pass of the mLSTM chunkwise kernel for AMD GPUs.
# Optimizations focus on:
# 1. Wavefront-aware block sizing (multiples of 64)
# 2. Improved memory access patterns for AMD's memory hierarchy
...
@triton.jit
def mlstm_chunkwise__parallel_fw_Hintra_kernel_amd(
    matQ,  # (B, NH, S, DHQK)
    matK,  # (B, NH, S, DHQK)
    matV,  # (B, NH, S, DHHV)
    ...
    AMD_OPTIM: tl.constexpr = True,  # Enable AMD optimizations by default
):
    ...
    # AMD-specific optimization: Use vectorized memory access where possible
    # This is especially beneficial for CDNA3.0 architecture
    ...
    # initialize accumulators - using directly float32 for better precision
    ...
    for idx_b_LKV in range(idx_b_LKV_end):
        ...
        for idx_b_DHQK in range(tl.cdiv(DHQK, siz_b_DHQK)):
            # AMD optimization: Aligned memory access for better performance
            ...
            # AMD optimization: Direct .to(DTYPE) conversion during load for better performance
            ...
            # AMD optimization: Use direct dot product which maps well to AMD's matrix instruction set
        ...
        # AMD optimization: Use fast max reduction
        vecM_new_val = tl.max(matDtilde_val, axis=1)  # row-wise max along siz_b_LKV
        vecM_new_val = tl.maximum(vecM_new_val, MINIMUM_MAX_VAL)  # element-wise max
        ...
        # AMD optimization: More efficient sum reduction
        vecN_intra_acc = vecM_ratio * vecN_intra_acc + tl.sum(matS, axis=1)

        # AMD optimization: Aligned memory access and coalesced memory loads
        ...
        # AMD optimization: Fast matrix multiply on AMD GPUs
    ...
    for idx_b_DHQK in range(tl.cdiv(DHQK, siz_b_DHQK)):
        # AMD optimization: Block pointers aligned to better match AMD's memory structure
        matQ_ptr = tl.make_block_ptr(
            base=matQ + idx_b_BNH * str_matQK_B_NH,
            shape=(S, DHQK),
            strides=(str_matQK_S, str_matQK_DHQK),
        ...
        # AMD optimization: Fuse multiple operations for better instruction efficiency
        ...
        # AMD optimization: Efficient matrix-matrix multiplication
    ...
    # compute the matH_comb (siz_b_LQ, siz_b_DHHV)
    # AMD optimization: Use vectorized division for better performance
    ...
    # AMD optimization: Store with proper data type conversion
    tl.store(matHout_ptr, matH_comb_val.to(OUTPUT_DTYPE), boundary_check=(0, 1))
