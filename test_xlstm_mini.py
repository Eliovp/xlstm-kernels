...
def run_test(kernel_type="stock"):
    ...
    if kernel_type == "stock":
        ...
        # Hybrid approach
        kernel_config = {
            "chunkwise_kernel": "chunkwise--native_autograd",  # Native that works on AMD
            "sequence_kernel": "native_sequence__native",      # Native that works on AMD
            "step_kernel": "native"                            # Native that works on AMD
        }
        print(f"\n===== Testing hybrid kernels =====")
...
# Run all tests
results = []
results.append(run_test("stock"))
results.append(run_test("amd"))
...
if len(results) >= 2:
    ...
    forward_speedup = stock['forward_time'] / amd['forward_time']
    gen_speedup = stock['gen_time'] / amd['gen_time']
    
    print("\n===== AMD OPTIMIZATION SPEEDUP =====")
